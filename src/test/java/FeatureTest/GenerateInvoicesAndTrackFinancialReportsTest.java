...
public class GenerateInvoicesAndTrackFinancialReportsTest {
	...
	public void multiple_customers_have_completed_purchases() {
		...
		meals1.add(salad);
		Order order1 = new Order(customer1, meals1);
		app.addOrder(order1);
		...
		meals2.add(salad);
		meals2.add(soup);
		Order order2 = new Order(customer2, meals2);
		app.addOrder(order2);
	...
	public void a_financial_report_is_generated_with_total_revenue_and_invoice_details() {
		...
		assertEquals("Total revenue should reflect all invoices", 30.0, lastFinancialReport.getTotalRevenue(), 0.01);
